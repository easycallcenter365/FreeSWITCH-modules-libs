...
static void worker_init(void* userdata) {
    // printf("setproctitle: [worker process] %s \n", g_main_ctx.program_name);
    setproctitle("[worker process] %s", g_main_ctx.program_name);
#ifdef OS_UNIX    
    signal(SIGNAL_RELOAD, signal_handler);
#endif
}
...
int master_workers_run(procedure_t worker_fn, void* worker_userdata,
    ...
    if (worker_processes == 0) {
        ...
        // master-workers processes
#ifdef OS_UNIX
        setproctitle("[master process] %s/%s", g_main_ctx.run_dir, g_main_ctx.program_name);
        //signal(SIGNAL_RELOAD, signal_handler);
#endif  

...
/**
*  child process will exit while recv parent process terminated event.
**/
void sigparent_handler(int signum) {
    printf("catch parent exit event: Due to parent process terminated. child process is now exiting, pid=%d.\n", getpid());
    exit(EXIT_SUCCESS);
}
...
static inline int hproc_spawn(proc_ctx_t* ctx) {
    ...
    if (pid < 0) {
        ...
        // add signal to monitor parent process exit event.
